...
export function NovedadDetalleClient({ novedad, slug }: Props) {
  ...
  const videoEmbedUrl = useMemo(() => {
    ...
    return null;
  }, [novedad.videoUrl, novedad.youtubeVideoId]);
  ...
  return (
    <main className="page-enter min-h-[60vh] bg-app">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-5 py-10 sm:px-8">
        ...
        <article
          data-reveal
          className="rounded-3xl border border-brand-dark/10 bg-surface p-6 shadow-[0_10px_26px_rgba(75,56,49,0.10)] sm:p-8"
        >
          {novedad.imagenPrincipal ? (
            <Image
              ...
              height={780}
              quality={62}
              sizes="(max-width: 640px) 92vw, (max-width: 1024px) 88vw, 896px"
          ...
          <h1 className="mt-3 text-3xl leading-tight font-black text-brand-dark sm:text-4xl">{novedad.titulo}</h1>
          ...
          <div className="mt-6 space-y-4 text-[15px] leading-relaxed text-brand-dark/85">
            ...
            {novedad.contenido ? <p className="whitespace-pre-line">{novedad.contenido}</p> : <p>Sin contenido ampliado para esta novedad.</p>}
          </div>
                ...
                {imagenesGaleria.map((imagen, idx) => (
                  ...
                  >
                    <Image
                      ...
                      height={700}
                      quality={58}
                      sizes="(max-width: 640px) 92vw, (max-width: 1024px) 44vw, 430px"
        ...
        >
          ...
          >
            ...
            <div className="relative h-[62vh] min-h-[280px] w-full">
              <Image src={imagenActiva} alt="Imagen de novedad" fill sizes="100vw" className="object-contain" />
            </div>
          </div>
        </div>
      ) : null}
