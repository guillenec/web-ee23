...
const BORRADOR_STORAGE_KEY = "ee23_admin_novedad_borrador_v3";

export default function AdminNovedadesCrearPage() {
    ...
    try {
      ...
      if (data.form) {
        setForm({
          ...
          galeriaPublicIds: Array.isArray(data.form.galeriaPublicIds) ? data.form.galeriaPublicIds : [],
        });
      }
  ...
  const guardarEnFirebase = async () => {
    ...
    try {
      ...
      setGuardadoOk(`Novedad guardada con ID: ${slugFinal}`);
      toast.success("Novedad guardada con exito");
      window.localStorage.removeItem(BORRADOR_STORAGE_KEY);
    } catch {
  ...
  const solicitarIa = async (payloadIa: {
          ...
          >
            ...
            <Campo label="Estado">
              <div className="grid gap-2 sm:grid-cols-2">
                <button
                  type="button"
                  onClick={() => setForm((prev) => ({ ...prev, estado: "publicado" }))}
